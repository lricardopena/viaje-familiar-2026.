...
- `storyland.html` — a separate, self-contained mini-app (own HTML/CSS/JS, own `localStorage` key `storyland_state_v1`) for the Story Land theme-park day. Linked from `index.html`'s Story Land activity card. Not part of the `index.html`/`data.js` split above — its small attraction array and scoring logic live together in this one file on purpose (see `specs/SPECIFICATIONS.md.asc` section 21 for the full spec: data model, scoring formula/weights, zone/proximity logic, "long line" cooldown).
...
- **`specs/SPECIFICATIONS.md.asc`** — architecture, design and content guardrails: repo/stack conventions, publishing checklist, UI conventions, editorial style rules, and the full technical spec of `storyland.html` (data model, scoring weights, zone/cooldown logic).
