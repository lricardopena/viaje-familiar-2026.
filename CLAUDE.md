...
- `specs/SPECIFICATIONS.md.asc` — encrypted with the same scheme and password as `HANDOVER.md.asc` (see `specs/README.md`). Architecture, design and content guardrails — the technical sibling of the handover. See "Decisions & specs" below.
...
Two encrypted documents track everything that isn't itself the itinerary content: `HANDOVER.md.asc` (root) and `specs/SPECIFICATIONS.md.asc`. Same GPG scheme, same password (symmetric AES-256, not stored in the repo — see either file's `README.md` for the decrypt/re-encrypt commands). They're split by kind of information, not by importance:
...
- **`specs/SPECIFICATIONS.md.asc`** — architecture, design and content guardrails: repo/stack conventions, publishing checklist, UI conventions, editorial style rules, and the full technical spec of the Theme Park Companion (`assets/theme-park-core.js` + `parks/*.js`; data model, scoring weights, zone/cooldown logic, per-child eligibility).
...
**Before decrypting/editing either file, ask the user whether to do it now or skip it for this session.** You do not have the passphrase and must not attempt to guess it, derive it, or reuse one seen in unrelated context. Updating either file means asking the user to paste the password into the conversation (it lands in that session's chat history — the same tradeoff noted in `HANDOVER.md.asc` section 62), so don't treat it as a routine step to do silently: surface the question, let the user say "skip" or provide the password, and if they skip, just proceed with the rest of the task and mention in your summary what would need recording later (so it isn't silently lost). Never invent content for either file without user input, and never rewrite a section you haven't read — decrypt first, edit, then re-encrypt with the same scheme (`gpg --symmetric --cipher-algo AES256 --armor`), and delete the plaintext copy before finishing.
