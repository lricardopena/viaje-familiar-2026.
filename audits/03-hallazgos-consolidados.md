...
| H6 | **P2 — ✅ RESUELTO (con matiz)** | T2.F | `assets/theme-park-core.js` (2518 líneas), `index.html` | Densidad baja de `aria-*` (6 usos en el core) relativa al volumen de UI dinámica generada; riesgo WCAG 4.1.2 no confirmado con lector de pantalla real | conteo grep, no verificado con herramienta de a11y dedicada | **Aplicado:** pase real con axe-core (`tests/accessibility/axe-audit.spec.js`, `npm run test:a11y`) contra ambos parques × 4 pestañas. La hipótesis de aria-* NO se confirmó (0 violaciones 4.1.2) — en cambio se encontraron y corrigieron 8 violaciones `serious` de contraste de color (WCAG 1.4.3), un hallazgo distinto que T2.F había marcado "no verificable". Ver `audits/2F-performance-a11y.md` §Actualización. | M |
