...
- **Leer es distinto de modificar.** Un agente puede consultar/descifrar cualquiera de estos documentos **proactivamente**, sin pedir confirmación, cuando sean relevantes para completar correctamente la tarea en curso — son documentación técnica necesaria para trabajar en el repo, no un secreto que requiera permiso para mirar. La passphrase debe obtenerse únicamente por un mecanismo legítimo ya disponible en el contexto/entorno autorizado del agente (nunca inventada, adivinada, ni pedida al usuario para que la pegue en el chat); si no hay forma legítima de obtenerla, el agente debe decirlo como una limitación concreta en vez de simular o inventar el contenido del documento. La passphrase en sí nunca debe imprimirse, registrarse (logs, commits, comentarios) ni exponerse de ninguna forma. El plaintext descifrado es siempre temporal: nunca se versiona, y se elimina del disco al terminar de leerlo/editarlo.
- **Modificar sí requiere criterio, no permiso previo.** Editar un `.md.asc` es apropiado cuando una decisión real cambia arquitectura, comportamiento, guardrails o el contrato `PARK` — no hace falta preguntarle al usuario antes de abrir el archivo para eso. El flujo: actualizar el documento canónico correspondiente (nunca duplicar la misma regla en varios documentos — ver la regla de fuente única más arriba), re-cifrar, verificar el roundtrip (descifrar de nuevo y comparar), y eliminar cualquier plaintext temporal antes de terminar.
